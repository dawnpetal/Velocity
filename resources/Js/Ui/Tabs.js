const tabs = (() => {
  ...
  function render() {
    ...
    strip
      .querySelectorAll(".tab[data-id]")
      .forEach((el) => renderedTabs.set(el.dataset.id, el));
    ...
    state.openTabIds.forEach((id) => {
      ...
      if (existingIcon) {
        const fresh = helpers.fileIconImg(file.name, false, false, 13);
        existingIcon.src = fresh.src;
      }
      if (isActive)
  ...
  function _createTab(id, file) {
    ...
    tab.className = "tab";
    tab.dataset.id = id;
    const icon = helpers.fileIconImg(file.name, false, false, 13);
