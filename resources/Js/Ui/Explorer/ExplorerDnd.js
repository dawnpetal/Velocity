const ExplorerDnd = (() => {
  ...
  function _showDropLine(row, edge) {
    ...
    ind.style.right = "4px";
    ind.style.top = ((edge === "before" ? rowRect.top : rowRect.bottom) - treeRect.top + tree.scrollTop) + "px";
  ...
  function attachFileDrag(row, node) { _attachNodeDrag(row, node); }
  function attachFolderDrag(row, node) { _attachNodeDrag(row, node); }
  ...
  function attachRootHeaderDrop(headerEl, rootNode) {
    ...
    headerEl.addEventListener("drop", async (e) => {
      ...
      if (e.dataTransfer.files.length > 0) {
        if (_handlingDrop) return;
        _handlingDrop = true;
        try { await _externalDrop(e.dataTransfer, rootNode.path); }
        finally { _handlingDrop = false; }
        return;
      }
    ...
    for (const dragNode of dragNodes) {
      ...
      if (zone === "into") {
        ...
        if (parent) {
          ...
          destDir = root ? root.path : targetNode.path.substring(0, targetNode.path.lastIndexOf("/"));
        }
      }
  ...
  async function _externalDrop(dt, destDir) {
    if (!destDir) { toast.show("Open a folder first", "warn"); return; }
    const items = Array.from(dt.items ?? []);
    const entries = items.map(i => i.webkitGetAsEntry?.()).filter(Boolean);
    if (!entries.length) return;

    toast.show(`Copying ${entries.length} item${entries.length > 1 ? "s" : ""}…`, "info", 2000);
    let copied = 0;
  ...
  async function _copyEntry(entry, destPath) {
    if (entry.isFile) {
      ...
      await window.__TAURI__.core.invoke("write_binary_file", { path: destPath, data: btoa(bin) });
    } else if (entry.isDirectory) {
      await window.__TAURI__.core.invoke("create_dir", { path: destPath }).catch(() => {});
      ...
      for (const child of all) await _copyEntry(child, `${destPath}/${child.name}`);
    }
  }

  function attachRootDrop(rootEl) {
    rootEl.addEventListener("dragenter", (e) => { e.preventDefault(); });
