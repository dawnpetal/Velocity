const guide = (() => {
  ...
  const CHARACTER_LINES = {
    'Walter White': {
      welcome: `I insist you complete this tutorial. Not because you need it, but because I need it. I'm Walter White, and this is my guide to VelocityUI. Let's get started.`,
      Execute: {
  ...
  const STEPS = [
    {
      ...
      text: () => {
        ...
        return `Hey, I'm ${_currentChar.name}. This is VelocityUI, a script editor made for Roblox. I'll walk you through every part of the app. Use the arrows to move around, click any dot to jump to a step, or press Space and Enter to go forward. Esc exits.`;
      },
    },
    {
    ...
    },
    {
      ...
      selector: '#fabWrap',
      padding: 10,
      text: "When you hit Execute, VelocityUI scans ports 6969 through 7069 looking for your executor's local HTTP server. Once it finds one, it sends the script over. That's why Roblox has to be running first, the executor starts that server when it attaches.",
    },
    {
      view: 'explorer',
    ...
    },
    {
      ...
      text: () => {
        ...
        if (line) return line.text;
        return "After you execute something, results show up here. The Output tab shows what VelocityUI got back, success messages, print output, or errors. The Console tab watches Roblox's own output log. Cmd+` toggles the panel open and closed. The trash icon clears it.";
      },
    },
    {
      view: 'explorer',
      ...
      selector: '#bottomPanel',
      padding: 2,
      text: "Red output means the script threw a Lua error. Usually something like 'attempt to index nil value', which is the script's fault, not VelocityUI's. The message and line number tell you what went wrong. Green means it ran fine.",
    },
    {
      view: 'explorer',
      label: 'Status Bar',
    ...
    },
    {
      ...
      selector: '#sp-folder',
      padding: 8,
      text: 'Shows your current workspace folder path. Open Folder lets you switch to a different one. VelocityUI remembers the last folder you had open and brings it back on next launch.',
    },
    {
      view: 'settings',
  ...
  function _applyCharacterImage(step) {
    ...
    const checkpointLine =
      step && step.checkpoint ? _getCharacterLine(_currentChar.name, step.checkpoint) : null;
  ...
  async function start() {
    ...
    if (_sidebar) {
      ...
      _sidebar.dataset.guideLocked = '1';
      _sidebar.dataset.guidePrevLocked = _sidebar.dataset.navLocked === '1' ? '1' : '0';
      delete _sidebar.dataset.navLocked;
      uiState.setSidebarHidden?.(false);
    }
  ...
  async function stop() {
    ...
    const _sidebarEl = document.querySelector('.sidebar[data-guide-locked]');
    if (_sidebarEl) {
      if (_sidebarEl.dataset.guidePrevLocked === '1') {
        _sidebarEl.dataset.navLocked = '1';
        _sidebarEl.classList.add('hidden');
        uiState.setSidebarHidden?.(true);
        uiState.setSidebarLocked?.(true);
      }
      delete _sidebarEl.dataset.guidePrevLocked;
      delete _sidebarEl.dataset.guideLocked;
    }
    _barEl = null;
    _dotStrip = null;
    _boundListeners = {};
