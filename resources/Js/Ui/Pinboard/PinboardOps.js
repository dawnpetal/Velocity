const PinboardOps = (() => {
  ...
  async function run(snippet, showOutput, context) {
    ...
    const statusEl = document.getElementById("pb-status-" + snippet.id);
    const card = document.querySelector(
      '.pb-card[data-id="' + snippet.id + '"]',
    );
    const runBtn = card ? card.querySelector(".pb-btn-run") : null;
    ...
    try {
      ...
      if (statusEl) {
        ...
        setTimeout(() => {
          if (statusEl) statusEl.className = "pb-status-bar";
  ...
  function openInEditor(snippet, context) {
    ...
    onRender();
    toast.show(
      'Editing "' + snippet.label + '" \u2014 \u2318S saves back to pinboard',
      "info",
      EDITOR_TOAST_DURATION,
    );
  }
  ...
  function isSnippetFile(fileId, context) {
    return [...context.activeEditorIds.values()].includes(fileId);
  }
  function pinFile(node, context) {
    const { snippets, onSave } = context;
