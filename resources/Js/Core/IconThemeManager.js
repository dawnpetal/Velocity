const iconThemeManager = (() => {
  ...
  async function isInstalled(id) {
    return await invoke("icon_theme_is_installed", { id });
  }

  async function isActive(id) {
    return await invoke("icon_theme_is_active", { id });
  }
  ...
  async function activate(id) {
    const ok = await invoke("icon_theme_activate", { id });
    if (ok) {
  ...
  async function loadInstalledIcons(themeId) {
    ...
    const result = await invoke("icon_theme_load_installed_icons", { themeId });
    if (!result) return null;
    const [iconsJson, iconDir] = result;
    return { iconsJson, iconDir };
  }

  async function install(pack, onProgress) {
    if (pack.builtin) {
      await invoke("icon_theme_activate", { id: pack.id });
      return true;
    }

    onProgress?.("Downloading and installing…");
    try {
      await invoke("icon_theme_install", { id: pack.id });
      onProgress?.("Done!");
      return true;
    } catch (e) {
  ...
  async function uninstall(id) {
    ...
    const ok = await invoke("icon_theme_uninstall", { id });
    if (ok) {
  ...
  function renderList() {
    ...
    Promise.all([getActive(), getInstalled()]).then(([activeId, installedSet]) => {
      for (const pack of _registry) {
        const row = document.createElement("div");
        row.className = "icon-theme-row" + (pack.id === activeId ? " active" : "");
